...
--
-- Host: 127.0.0.1
-- Generation Time: May 24, 2024 at 08:51 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddCertificationTemplate` (
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddCitizen` (IN `FirstName` VARCHAR(50), IN `LastName` VARCHAR(50), IN `Address` VARCHAR(255), IN `Contact` VARCHAR(20))   BEGIN
    INSERT INTO Citizen (Citizen_fname, Citizen_lname, Address, Contact_NO)
    VALUES (FirstName, LastName, Address, Contact);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddFeeType` (IN `FeeTypeName` VARCHAR(100), IN `FeeAmount` DECIMAL, IN `CollectionType` VARCHAR(50), IN `CollectionPeriod` VARCHAR(50), IN `CollectionStart` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewBarangay` (IN `p_BarangayName` VARCHAR(100), IN `p_Location` VARCHAR(255))   BEGIN
    INSERT INTO Barangay (BarangayName, Location)
    VALUES (p_BarangayName, p_Location);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewBarangayCaptain` (IN `p_CaptainName` VARCHAR(100), IN `p_Address` VARCHAR(255), IN `p_StartDate` DATE)   BEGIN
    INSERT INTO Barangay_Captain (CaptainName, Address, StartDate)
    VALUES (p_CaptainName, p_Address, p_StartDate);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewPurok` (IN `p_PurokName` VARCHAR(100), IN `p_BarangayID` INT)   BEGIN
    INSERT INTO Purok (PurokName, BarangayID)
    VALUES (p_PurokName, p_BarangayID);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewReport` (IN `p_ReportDetails` TEXT, IN `p_ReportDate` DATE)   BEGIN
    INSERT INTO Reports (ReportDetails, ReportDate)
    VALUES (p_ReportDetails, p_ReportDate);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddNewStaff` (IN `p_StaffName` VARCHAR(100), IN `p_Position` VARCHAR(100))   BEGIN
    INSERT INTO Staff (StaffName, Position)
    VALUES (p_StaffName, p_Position);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `AddPurok` (IN `PurokName` VARCHAR(100), IN `PurokAddress` VARCHAR(255), IN `PurokLeader` VARCHAR(100), IN `PurokPopulation` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddReport` (IN `ReportTitle` VARCHAR(255), IN `ReportDetails` TEXT, IN `ReportDate` DATE)   BEGIN
    INSERT INTO Reports (Report_title, Report_details, Report_date)
    VALUES (ReportTitle, ReportDetails, ReportDate);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteCitizen` (IN `p_CitizenID` INT)   BEGIN
    DELETE FROM Citizen
    WHERE CitizenID = p_CitizenID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteStaffMember` (IN `p_StaffID` INT)   BEGIN
    DELETE FROM Staff
    WHERE StaffID = p_StaffID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCitizenBySurname` (IN `Surname` VARCHAR(100))   BEGIN
    SELECT * FROM Citizen WHERE Citizen_lname LIKE CONCAT('%', Surname, '%');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCitizensInBarangay` (IN `inputBarangayID` INT)   BEGIN
    SELECT * FROM Citizen WHERE Barangay_id = inputBarangayID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetPurokInfo` (IN `PurokID` INT)   BEGIN
    SELECT *
    FROM Purok
    WHERE Purok_ID = PurokID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertFeeTransaction` (IN `amount` DECIMAL(10,2), IN `description` TEXT, IN `citizen_id` INT)   BEGIN
    INSERT INTO Fees (Amount, Description, Citizen_ID) VALUES (amount, description, citizen_id);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertHouseholdMember` (IN `household_id` INT, IN `member_name` VARCHAR(100), IN `member_relation` VARCHAR(100))   BEGIN
    INSERT INTO HouseholdMembers (Household_ID, Member_Name, Member_Relation) VALUES (household_id, member_name, member_relation);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertTransaction` (IN `staff_name` VARCHAR(100), IN `transaction_citizen` VARCHAR(100), IN `trans_date` DATE, IN `trans_time` TIME, IN `trans_status` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ListResidentsWithActiveStatus` ()   BEGIN
    SELECT *
    FROM Residency_history
    WHERE Residency_status = 'Active';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListTransactionsInDateRange` (IN `StartDate` DATE, IN `EndDate` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `RetrieveHouseholdInfoByHouseholdID` (IN `HouseholdID` INT)   BEGIN
    SELECT *
    FROM Household
    WHERE Household_No = HouseholdID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `RetrieveIndividualsBySurname` (IN `LastName` VARCHAR(50))   BEGIN
    SELECT *
    FROM Citizen
    WHERE Citizen_lname = LastName;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `RetrieveIndividualsInSpecificBarangay` (IN `BarangayID` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `RetrieveReportsForSpecificAddress` (IN `SpecificAddress` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateBusinessStatus` (IN `business_id` INT, IN `new_status` VARCHAR(50))   BEGIN
    UPDATE Business SET Business_Status = new_status WHERE Business_ID = business_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateCitizen` (IN `p_CitizenID` INT, IN `p_CitizenName` VARCHAR(100), IN `p_Address` VARCHAR(255), IN `p_Contact` VARCHAR(50))   BEGIN
    UPDATE Citizen
    SET CitizenName = p_CitizenName, Address = p_Address, Contact = p_Contact
    WHERE CitizenID = p_CitizenID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateStaff` (IN `StaffID` INT, IN `NewFirstName` VARCHAR(50), IN `NewLastName` VARCHAR(50), IN `NewAddress` VARCHAR(255), IN `NewContact` VARCHAR(20))   BEGIN
    UPDATE Staff
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `CalculateResidenceDuration` (`p_EntryDate` DATE) RETURNS INT(11)  BEGIN 
    DECLARE residence_duration INT;
    SET residence_duration = DATEDIFF(CURDATE(), p_EntryDate);
    RETURN residence_duration;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `CalculateTotalFeesPaid` (`citizen_id` INT) RETURNS DECIMAL(10,2)  BEGIN
    DECLARE total DECIMAL(10, 2);
    SELECT SUM(AmountPaid) INTO total FROM Fees WHERE Citizen_ID = citizen_id;
    RETURN total;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `CalculateTotalRevenue` (`IRA` INT, `Donations` INT, `sk_fund` INT, `rpt_share` INT, `fees_and_charges` INT, `others` INT) RETURNS DECIMAL(10,2)  BEGIN
    DECLARE total_revenue DECIMAL(10,2);
    SET total_revenue = IRA + Donations + sk_fund + rpt_share + fees_and_charges + others;
    RETURN total_revenue;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `CalculateYearsOfService` (`StartDate` DATE) RETURNS INT(11)  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `CheckSeniorCitizenStatus` (`Birthdate` DATE) RETURNS TINYINT(1)  BEGIN
    DECLARE is_senior BOOLEAN;
    SET is_senior = (YEAR(CURRENT_DATE()) - YEAR(Birthdate)) >= 60;
    RETURN is_senior;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `GenerateFullName` (`FirstName` VARCHAR(50), `MiddleName` VARCHAR(50), `LastName` VARCHAR(50)) RETURNS VARCHAR(150) CHARSET utf8mb4 COLLATE utf8mb4_general_ci  BEGIN
    DECLARE full_name VARCHAR(150);
    SET full_name = CONCAT(FirstName, ' ', COALESCE(MiddleName, ''), ' ', LastName);
    RETURN full_name;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `GetBasicMemberInfo` (`p_MemberID` INT) RETURNS VARCHAR(255) CHARSET utf8mb4 COLLATE utf8mb4_general_ci  BEGIN 
...
CREATE DEFINER=`root`@`localhost` FUNCTION `RetrieveHouseholdMembersCount` (`HouseholdNo` INT) RETURNS INT(11)  BEGIN
    DECLARE member_count INT;
    SELECT COUNT(*) INTO member_count FROM Household WHERE Household_No = HouseholdNo;
    RETURN member_count;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `RetrieveTransactionCountByCitizen` (`citizen_id` INT) RETURNS INT(11)  BEGIN
...
--
-- Dumping data for table `barangay_admin`
--
...
--
-- Dumping data for table `barangay_archives`
--
...
CREATE TABLE `barangay_facilities` (
  ...
  `Facility_Access_Lvl` varchar(100) DEFAULT NULL,
...
CREATE TABLE `barangay_profile` (
  ...
  `Brgy_telefax` int(11) DEFAULT NULL,
  `Brgy_Email` varchar(100) DEFAULT NULL,
  `Brgy_website` varchar(255) DEFAULT NULL,
  `Brgy_logo` varchar(255) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `barangay_properties` (
  ...
  `Property_remark` text DEFAULT NULL,
  `Brgy_ID` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `barangay_properties`
--
...
INSERT INTO `barangay_workers` (`Worker_ID`, `Worker_fname`, `Worker_mname`, `Worker_lname`, `Accesslvl`, `status`, `Username`, `password`, `remember_token`, `Profile_pic`, `created_at`, `updated_at`) VALUES
(1, 'John', 'Doe', 'Smith', 0, 1, 'johnsmith', '$2y$12$CBYcAD2xioSCyEFvrmujfuzTbCkiF5rhagvZrlqLgiPx5fWgfmkLC', 'S9CUD4EtJ9j8rvEtI6OoGFCSsCj9RYF7xLZyqrYYpHYRccuw0CCYLkmwVZ7g', 'profile_pic_1.jpg', NULL, NULL),
(2, 'Jane', 'Mary', 'Doe', 1, 1, 'janedoe', '$2y$12$vWDlmCV2Ia6O4FI3QBxbceui4ZLi8LBtcVDdpHFLhoaR7vCPNu.h.', 'p3bKriGZbiqGtXOmHTuZ1tJDZydr9mPB1A6SpVK0oWZVGgoNl6VA5tcWKb2m', 'profile_pic_2.jpg', NULL, NULL),
(3, 'Michael', 'James', 'Johnson', 1, NULL, 'michaeljohnson', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_3.jpg', NULL, NULL),
(4, 'Emily', 'Grace', 'Williams', 3, NULL, 'emilywilliams', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_4.jpg', NULL, NULL),
(5, 'Matthew', 'David', 'Brown', 1, NULL, 'matthewbrown', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_5.jpg', NULL, NULL),
(6, 'Olivia', 'Rose', 'Martinez', 2, NULL, 'oliviamartinez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_6.jpg', NULL, NULL),
(7, 'Ethan', 'Daniel', 'Garcia', 1, NULL, 'ethangarcia', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_7.jpg', NULL, NULL),
(8, 'Sophia', 'Madison', 'Lopez', 3, NULL, 'sophialopez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_8.jpg', NULL, NULL),
(9, 'Noah', 'Anthony', 'Gonzalez', 1, NULL, 'noahgonzalez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_9.jpg', NULL, NULL),
(10, 'Emma', 'Chloe', 'Rodriguez', 2, NULL, 'emmarodriguez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_10.jpg', NULL, NULL),
(11, 'William', 'Alexander', 'Perez', 1, NULL, 'williamperez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_11.jpg', NULL, NULL),
(12, 'Isabella', 'Zoe', 'Hernandez', 3, NULL, 'isabellahernandez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_12.jpg', NULL, NULL),
(13, 'James', 'Gabriel', 'Flores', 1, NULL, 'jamesflores', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_13.jpg', NULL, NULL),
(14, 'Ava', 'Penelope', 'Washington', 2, NULL, 'avawashington', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_14.jpg', NULL, NULL),
(15, 'Alexander', 'Ryan', 'Taylor', 1, NULL, 'alexandertaylor', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_15.jpg', NULL, NULL),
(16, 'Mia', 'Avery', 'Adams', 3, NULL, 'miaadams', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_16.jpg', NULL, NULL),
(17, 'Liam', 'Nathaniel', 'Campbell', 1, NULL, 'liamcampbell', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_17.jpg', NULL, NULL),
(18, 'Charlotte', 'Hannah', 'Green', 2, NULL, 'charlottegreen', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_18.jpg', NULL, NULL),
(19, 'Benjamin', 'Christopher', 'Evans', 1, NULL, 'benjaminevans', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_19.jpg', NULL, NULL),
(20, 'Amelia', 'Lily', 'Harris', 3, NULL, 'ameliaharris', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_20.jpg', NULL, NULL),
(21, 'Daniel', 'Christian', 'Martinez', 1, NULL, 'danielmartinez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_21.jpg', NULL, NULL),
(22, 'Harper', 'Natalie', 'Lopez', 2, NULL, 'harperlopez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_22.jpg', NULL, NULL),
(23, 'Logan', 'Dylan', 'Gonzalez', 1, NULL, 'logangonzalez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_23.jpg', NULL, NULL),
(24, 'Evelyn', 'Sofia', 'Flores', 3, NULL, 'evelynflores', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_24.jpg', NULL, NULL),
(25, 'Sebastian', 'Joseph', 'Washington', 1, NULL, 'sebastianwashington', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_25.jpg', NULL, NULL),
(26, 'Madison', 'Ava', 'Adams', 2, NULL, 'madisonadams', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_26.jpg', NULL, NULL),
(27, 'Jackson', 'Lucas', 'Garcia', 1, NULL, 'jacksongarcia', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_27.jpg', NULL, NULL),
(28, 'Avery', 'Mia', 'Lopez', 3, NULL, 'averylopez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_28.jpg', NULL, NULL),
(29, 'Jack', 'Noah', 'Gonzalez', 1, NULL, 'jackgonzalez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_29.jpg', NULL, NULL),
(30, 'Lily', 'Amelia', 'Rodriguez', 2, NULL, 'lilyrodriguez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_30.jpg', NULL, NULL),
(31, 'Ryan', 'Alexander', 'Perez', 1, NULL, 'ryanperez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_31.jpg', NULL, NULL),
(32, 'Zoe', 'Isabella', 'Hernandez', 3, NULL, 'zoehernandez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_32.jpg', NULL, NULL),
(33, 'Gabriel', 'James', 'Flores', 1, NULL, 'gabrielflores', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_33.jpg', NULL, NULL),
(34, 'Penelope', 'Ava', 'Washington', 2, NULL, 'penelopewashington', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_34.jpg', NULL, NULL),
(35, 'Joseph', 'Sebastian', 'Taylor', 1, NULL, 'josephtaylor', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_35.jpg', NULL, NULL),
(36, 'Avery', 'Mia', 'Adams', 3, NULL, 'averyadams', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_36.jpg', NULL, NULL),
(37, 'Lucas', 'Jackson', 'Garcia', 1, NULL, 'lucasgarcia', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_37.jpg', NULL, NULL),
(38, 'Natalie', 'Harper', 'Lopez', 2, NULL, 'natalielopez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_38.jpg', NULL, NULL),
(39, 'Noah', 'Benjamin', 'Gonzalez', 1, NULL, 'noahgonzalez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_39.jpg', NULL, NULL),
(40, 'Chloe', 'Emma', 'Rodriguez', 3, NULL, 'chloerodriguez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_40.jpg', NULL, NULL),
(41, 'Alexander', 'William', 'Perez', 1, NULL, 'alexanderperez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_41.jpg', NULL, NULL),
(42, 'Zoe', 'Isabella', 'Hernandez', 2, NULL, 'zoehernandez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_42.jpg', NULL, NULL),
(43, 'Gabriel', 'James', 'Flores', 1, NULL, 'gabrielflores', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_43.jpg', NULL, NULL),
(44, 'Penelope', 'Ava', 'Washington', 3, NULL, 'penelopewashington', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_44.jpg', NULL, NULL),
(45, 'Joseph', 'Sebastian', 'Taylor', 1, NULL, 'josephtaylor', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_45.jpg', NULL, NULL),
(46, 'Avery', 'Mia', 'Adams', 2, NULL, 'averyadams', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_46.jpg', NULL, NULL),
(47, 'Lucas', 'Jackson', 'Garcia', 1, NULL, 'lucasgarcia', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_47.jpg', NULL, NULL),
(48, 'Natalie', 'Harper', 'Lopez', 3, NULL, 'natalielopez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_48.jpg', NULL, NULL),
(49, 'Noah', 'Benjamin', 'Gonzalez', 1, NULL, 'noahgonzalez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_49.jpg', NULL, NULL),
(50, 'Chloe', 'Emma', 'Rodriguez', 2, NULL, 'chloerodriguez', '482c811da5d5b4bc6d497ffa98491e38', NULL, 'profile_pic_50.jpg', NULL, NULL),
(52, 'Carl Justin', 'Cabuntocan', 'Efren', 0, 1, 'xeejay', '$2y$12$bYH8OWVJKRDqJwMVCS0MZeijvPjab8Sx/MYrwjXUvORN.lDMhm1wi', 'K8ucBecodtpJCGzY1pAQlOpqUimXYjXL9TnyrvOnz7YTWqZZQAxSMSHxk4rb', 'TWOzakEAmYr8OH7MNWpfdirPGNeREt.png', '2024-05-21 01:47:13', '2024-05-21 11:34:01'),
(54, 'Lee Christian', 'Ala', 'Amor', 0, 1, 'amor', '$2y$12$qxP9OnFHExL7qbFY6lbV3.R0vSHWQgbrQ0KlUZYnTdyFR0a464hWi', NULL, NULL, '2024-05-21 03:00:57', '2024-05-21 03:00:57'),
(59, 'Jay', 'Lalat', 'Marasigan', 0, 1, 'jaylalat', '$2y$12$jAQ9wsJvBP.Px.2zK.3LBOkAasoH.DVDSQ6ouYstbSMOEU5xScymm', 'MZ00XPEvN1nwwjDTOo5EN7pMwwP5xaAd7sEjRIPZGK8Q6HFM4EJEuEViFQzZ', 'b5XwcWVxxtW1Cg9MjmHlfjOYgc83S3.png', '2024-05-21 09:30:40', '2024-05-21 09:35:59');

-- --------------------------------------------------------

--
-- Stand-in structure for view `basicinformationofmembers`
-- (See below for the actual view)
--
CREATE TABLE `basicinformationofmembers` (
...
CREATE TABLE `brgy_geography` (
  ...
  `Brgy_category` varchar(50) DEFAULT NULL,
  `Land_class` varchar(50) DEFAULT NULL,
  `Brgy_location` varchar(100) DEFAULT NULL,
  `MES` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `brgy_geography`
--
...
CREATE TABLE `business` (
  ...
  `Business_purok` varchar(100) DEFAULT NULL,
  `Business_permit` varchar(255) DEFAULT NULL,
  `Contact_NO` varchar(20) DEFAULT NULL,
  `BOF` varchar(255) DEFAULT NULL,
  `Business_Type` varchar(100) DEFAULT NULL,
  `Business_permit_copy` varchar(100) DEFAULT NULL,
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `business` (`Business_ID`, `Business_name`, `Owner_name`, `Date_established`, `Business_Status`, `Business_Address`, `Business_purok`, `Business_permit`, `Contact_NO`, `BOF`, `Business_Type`, `Business_permit_copy`, `created_at`, `updated_at`) VALUES
(1, 'ABC Bakery', 'John Smith', '2020-01-15', 'Active', '123 Main Street', 'Purok 1', 'BP202012345', '123-456-7890', 'License', 'Bakery', 'BP202012345_copy.pdf', NULL, NULL),
(2, 'XYZ Pharmacy', 'Emily Johnson', '2019-05-20', 'Active', '456 Elm Street', 'Purok 2', 'PP202112345', '234-567-8901', 'Permit', 'Pharmacy', 'PP202112345_copy.pdf', NULL, NULL),
(3, 'Sunrise Cafe', 'Michael Williams', '2021-08-10', 'Active', '789 Oak Street', 'Purok 3', 'BP202212345', '345-678-9012', 'License', 'Cafe', 'BP202212345_copy.pdf', NULL, NULL),
(4, 'Green Thumb Nursery', 'Jessica Brown', '2018-11-05', 'Inactive', '101 Pine Street', 'Purok 4', 'NP202312345', '456-789-0123', 'N/A', 'Nursery', 'NP202312345_copy.pdf', NULL, NULL),
(5, 'Tech Solutions', 'Matthew Davis', '2022-03-25', 'Active', '202 Maple Street', 'Purok 5', 'BP202412345', '567-890-1234', 'License', 'Technology', 'BP202412345_copy.pdf', NULL, NULL),
(6, 'Golden Gym', 'Sarah Taylor', '2017-09-30', 'Active', '303 Cedar Street', 'Purok 6', 'BP202512345', '678-901-2345', 'License', 'Gym', 'BP202512345_copy.pdf', NULL, NULL),
(7, 'Tasty Treats Restaurant', 'Daniel Martinez', '2020-06-12', 'Active', '404 Walnut Street', 'Purok 7', 'BP202612345', '789-012-3456', 'License', 'Restaurant', 'BP202612345_copy.pdf', NULL, NULL),
(8, 'Fresh Farms Market', 'Olivia Garcia', '2019-02-18', 'Active', '505 Cherry Street', 'Purok 8', 'BP202712345', '890-123-4567', 'License', 'Market', 'BP202712345_copy.pdf', NULL, NULL),
(9, 'Happy Tails Pet Grooming', 'Ethan Rodriguez', '2021-07-08', 'Active', '606 Peach Street', 'Purok 9', 'BP202812345', '901-234-5678', 'License', 'Pet Care', 'BP202812345_copy.pdf', NULL, NULL),
(10, 'Sunny Days Daycare', 'Sophia Martinez', '2018-04-03', 'Active', '707 Plum Street', 'Purok 10', 'BP202912345', '012-345-6789', 'License', 'Daycare', 'BP202912345_copy.pdf', NULL, NULL),
(11, 'ABC Auto Repair', 'Noah Hernandez', '2019-11-22', 'Active', '808 Grape Street', 'Purok 11', 'BP203012345', '123-456-7890', 'License', 'Auto Repair', 'BP203012345_copy.pdf', NULL, NULL),
(12, 'Green Clean Laundry', 'Emma Lopez', '2020-10-17', 'Active', '909 Lemon Street', 'Purok 12', 'BP203112345', '234-567-8901', 'License', 'Laundry', 'BP203112345_copy.pdf', NULL, NULL),
(13, 'Healthy Bites Cafe', 'Liam Martinez', '2019-06-29', 'Active', '010 Banana Street', 'Purok 13', 'BP203212345', '345-678-9012', 'License', 'Cafe', 'BP203212345_copy.pdf', NULL, NULL),
(14, 'City Fitness Center', 'Mia Gonzalez', '2021-04-14', 'Active', '111 Orange Street', 'Purok 14', 'BP203312345', '456-789-0123', 'License', 'Gym', 'BP203312345_copy.pdf', NULL, NULL),
(15, 'All Seasons Landscaping', 'Lucas Perez', '2018-12-05', 'Inactive', '212 Lime Street', 'Purok 15', 'NP203412345', '567-890-1234', 'N/A', 'Landscaping', 'NP203412345_copy.pdf', NULL, NULL),
(16, 'ABC Accounting Services', 'Zoe Hernandez', '2022-01-30', 'Active', '313 Kiwi Street', 'Purok 16', 'BP203512345', '678-901-2345', 'License', 'Accounting', 'BP203512345_copy.pdf', NULL, NULL),
(17, 'Silver Screen Cinema', 'Elijah Rodriguez', '2017-08-19', 'Active', '414 Grapefruit Street', 'Purok 17', 'BP203612345', '789-012-3456', 'License', 'Cinema', 'BP203612345_copy.pdf', NULL, NULL),
(18, 'Quick Bites Fast Food', 'Ava Martinez', '2020-05-10', 'Active', '515 Tomato Street', 'Purok 18', 'BP203712345', '890-123-4567', 'License', 'Fast Food', 'BP203712345_copy.pdf', NULL, NULL),
(19, 'ABC Plumbing Services', 'Logan Gonzalez', '2019-01-25', 'Active', '616 Pineapple Street', 'Purok 19', 'BP203812345', '901-234-5678', 'License', 'Plumbing', 'BP203812345_copy.pdf', NULL, NULL),
(20, 'Cozy Homes Real Estate', 'Harper Perez', '2021-08-14', 'Active', '717 Potato Street', 'Purok 20', 'BP203912345', '012-345-6789', 'License', 'Real Estate', 'BP203912345_copy.pdf', NULL, NULL),
(21, 'ABC Law Firm', 'Evelyn Hernandez', '2018-03-05', 'Active', '818 Cucumber Street', 'Purok 21', 'BP204012345', '123-456-7890', 'License', 'Law Firm', 'BP204012345_copy.pdf', NULL, NULL),
(22, 'Glamour Hair Salon', 'Logan Martinez', '2022-02-28', 'Active', '919 Radish Street', 'Purok 22', 'BP204112345', '234-567-8901', 'License', 'Hair Salon', 'BP204112345_copy.pdf', NULL, NULL),
(23, 'ABC Consulting Group', 'Sophia Hernandez', '2017-10-20', 'Active', '020 Carrot Street', 'Purok 23', 'BP204212345', '345-678-9012', 'License', 'Consulting', 'BP204212345_copy.pdf', NULL, NULL),
(24, 'Sunny Skies Travel Agency', 'Lucas Garcia', '2020-07-15', 'Active', '121 Turnip Street', 'Purok 24', 'BP204312345', '456-789-0123', 'License', 'Travel Agency', 'BP204312345_copy.pdf', NULL, NULL),
(25, 'ABC Catering Services', 'Lily Lopez', '2019-04-08', 'Active', '222 Onion Street', 'Purok 25', 'BP204412345', '567-890-1234', 'License', 'Catering', 'BP204412345_copy.pdf', NULL, NULL),
(26, 'Smart Solutions IT Services', 'Mason Perez', '2021-01-03', 'Active', '323 Eggplant Street', 'Purok 26', 'BP204512345', '678-901-2345', 'License', 'IT Services', 'BP204512345_copy.pdf', NULL, NULL),
(27, 'Golden Years Retirement Home', 'Aria Gonzalez', '2018-06-18', 'Active', '424 Squash Street', 'Purok 27', 'BP204612345', '789-012-3456', 'License', 'Retirement Home', 'BP204612345_copy.pdf', NULL, NULL),
(28, 'ABC Auto Sales', 'Eli Martinez', '2022-05-13', 'Active', '525 Zucchini Street', 'Purok 28', 'BP204712345', '890-123-4567', 'License', 'Auto Sales', 'BP204712345_copy.pdf', NULL, NULL),
(29, 'Natures Bounty Health Store', 'Nova Hernandez', '2017-12-01', 'Active', '626 Radish Street', 'Purok 29', 'BP204812345', '901-234-5678', 'License', 'Health Store', 'BP204812345_copy.pdf', NULL, NULL),
(30, 'ABC Construction Company', 'Sofia Garcia', '2020-09-26', 'Active', '727 Carrot Street', 'Purok 30', 'BP204912345', '012-345-6789', 'License', 'Construction', 'BP204912345_copy.pdf', NULL, NULL),
(31, 'Fit & Fab Fitness Center', 'Ethan Lopez', '2019-06-18', 'Active', '828 Turnip Street', 'Purok 31', 'BP205012345', '123-456-7890', 'License', 'Fitness Center', 'BP205012345_copy.pdf', NULL, NULL),
(32, 'ABC Marketing Solutions', 'Scarlett Perez', '2022-03-11', 'Active', '929 Onion Street', 'Purok 32', 'BP205112345', '234-567-8901', 'License', 'Marketing', 'BP205112345_copy.pdf', NULL, NULL),
(33, 'Sunny Side Up Breakfast Cafe', 'Henry Martinez', '2018-11-05', 'Active', '030 Eggplant Street', 'Purok 33', 'BP205212345', '345-678-9012', 'License', 'Cafe', 'BP205212345_copy.pdf', NULL, NULL),
(34, 'ABC Legal Services', 'Aurora Gonzalez', '2021-08-30', 'Active', '131 Squash Street', 'Purok 34', 'BP205312345', '456-789-0123', 'License', 'Legal Services', 'BP205312345_copy.pdf', NULL, NULL),
(35, 'Garden of Eden Florist', 'Leo Martinez', '2017-04-25', 'Active', '232 Zucchini Street', 'Purok 35', 'BP205412345', '567-890-1234', 'License', 'Florist', 'BP205412345_copy.pdf', NULL, NULL),
(36, 'ABC Software Solutions', 'Hazel Garcia', '2020-01-20', 'Active', '333 Radish Street', 'Purok 36', 'BP205512345', '678-901-2345', 'License', 'Software', 'BP205512345_copy.pdf', NULL, NULL),
(37, 'ABC Veterinary Clinic', 'Jack Lopez', '2017-10-15', 'Active', '434 Carrot Street', 'Purok 37', 'BP205612345', '789-012-3456', 'License', 'Veterinary', 'BP205612345_copy.pdf', NULL, NULL),
(38, 'ABC Bookkeeping Services', 'Luna Perez', '2021-07-10', 'Active', '535 Turnip Street', 'Purok 38', 'BP205712345', '890-123-4567', 'License', 'Bookkeeping', 'BP205712345_copy.pdf', NULL, NULL),
(39, 'ABC Art Studio', 'Kai Hernandez', '2018-04-05', 'Active', '636 Onion Street', 'Purok 39', 'BP205812345', '901-234-5678', 'License', 'Art Studio', 'BP205812345_copy.pdf', NULL, NULL),
(40, 'ABC Dental Clinic', 'Nova Garcia', '2022-01-30', 'Active', '737 Eggplant Street', 'Purok 40', 'BP205912345', '012-345-6789', 'License', 'Dental Clinic', 'BP205912345_copy.pdf', NULL, NULL),
(41, 'ABC Childcare Center', 'Scarlett Lopez', '2017-08-25', 'Active', '838 Radish Street', 'Purok 41', 'BP206012345', '123-456-7890', 'License', 'Childcare', 'BP206012345_copy.pdf', NULL, NULL),
(42, 'ABC Fashion Boutique', 'Hazel Perez', '2020-05-20', 'Active', '939 Carrot Street', 'Purok 42', 'BP206112345', '234-567-8901', 'License', 'Fashion Boutique', 'BP206112345_copy.pdf', NULL, NULL),
(43, 'ABC Security Services', 'Leo Martinez', '2019-02-14', 'Active', '040 Turnip Street', 'Purok 43', 'BP206212345', '345-678-9012', 'License', 'Security Services', 'BP206212345_copy.pdf', NULL, NULL),
(44, 'ABC Bakery', 'Luna Garcia', '2021-11-10', 'Active', '141 Onion Street', 'Purok 44', 'BP206312345', '456-789-0123', 'License', 'Bakery', 'BP206312345_copy.pdf', NULL, NULL),
(45, 'ABC Plumbing Services', 'Kai Hernandez', '2018-08-05', 'Active', '242 Eggplant Street', 'Purok 45', 'BP206412345', '567-890-1234', 'License', 'Plumbing', 'BP206412345_copy.pdf', NULL, NULL),
(46, 'ABC Marketing Solutions', 'Nova Garcia', '2020-03-01', 'Active', '343 Radish Street', 'Purok 46', 'BP206512345', '678-901-2345', 'License', 'Marketing', 'BP206512345_copy.pdf', NULL, NULL),
(47, 'ABC Dental Clinic', 'Scarlett Lopez', '2017-10-26', 'Active', '444 Carrot Street', 'Purok 47', 'BP206612345', '789-012-3456', 'License', 'Dental Clinic', 'BP206612345_copy.pdf', NULL, NULL),
(48, 'ABC Childcare Center', 'Hazel Perez', '2021-07-20', 'Active', '545 Turnip Street', 'Purok 48', 'BP206712345', '890-123-4567', 'License', 'Childcare', 'BP206712345_copy.pdf', NULL, NULL),
(49, 'ABC Fashion Boutique', 'Leo Martinez', '2018-04-15', 'Active', '646 Onion Street', 'Purok 49', 'BP206812345', '901-234-5678', 'License', 'Fashion Boutique', 'BP206812345_copy.pdf', NULL, NULL),
(50, 'ABC Security Services', 'Luna Garcia', '2022-01-10', 'Active', '747 Eggplant Street', 'Purok 50', 'BP206912345', '012-345-6789', 'License', 'Security Services', 'BP206912345_copy.pdf', NULL, NULL),
(70, 'Amor Eatry', 'Amor Lee', '2021-12-18', 'Active', 'Toril Davao City', 'Purok2', '102190', '091978821192', 'Services', 'Sole_Proprietorship', NULL, '2024-05-21 07:48:32', '2024-05-21 07:48:32');
...
CREATE TABLE `citizen` (
  ...
  `Citizen_profile` varchar(255) DEFAULT NULL,
  `Citizen_fname` varchar(50) NOT NULL,
  `Citizen_mname` varchar(50) DEFAULT NULL,
  `Citizen_lname` varchar(50) NOT NULL,
  ...
  `Senior_member` varchar(5) DEFAULT NULL,
  ...
  `Barangay_id` varchar(11) DEFAULT NULL,
  `PhilSys_Copy` varchar(255) NOT NULL,
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
(36, NULL, 'Avery', 'Mia', 'Adama', '7475 Pine Lane', 'Purok 36', 'House', 'F', 'AB-', 'Single', '1991-10-25', '0', 'Master\'s Degree', 'Graduate', 'Engineer', 'Employed', '444-555-6666', NULL, 'Christian', 'No', 'Yes', 'Yes', '90123', '36', 'PhilSysDoc36.pdf', NULL, NULL),
...
(46, NULL, 'Averys', 'Mia', 'Adams', '9495 Pine Lane', 'Purok 46', 'House', 'F', 'AB-', 'Single', '1991-10-25', '0', 'Master\'s Degree', 'Graduate', 'Engineer', 'Employed', '444-555-6666', NULL, 'Christian', 'No', 'Yes', 'Yes', '90123', '46', 'PhilSysDoc46.pdf', NULL, NULL),
...
(51, 'zMhHtmqaxW1fUHjV7c5jJDwLmR9imG.png', 'Carl Justin', 'Cabuntocan', 'Efren', 'Bayugan City', 'Purok1', 'Owned', 'Male', 'A+', 'Single', '2003-08-05', 'No', '', 'Yes', 'IT', 'No', '09197994188', '', 'Roman Catholic', 'No', 'N/A', 'Yes', '2532', '91', '', '2024-05-20 10:40:20', '2024-05-23 02:37:36'),
(52, '2sVMqNkVJ3aw44f7RFPTWzAObeNuEh.jpg', 'Cara', 'Cabuntocan', 'Almidela', 'Cavite', 'Purok2', 'Owned', 'Female', 'AB+', 'Married', '2002-07-31', 'No', 'College', 'Yes', 'Nurse', 'No', '09197991233', '', 'Roman Catholic', 'No', 'N/A', 'Yes', '253233', '1231231', '', '2024-05-20 13:15:26', '2024-05-20 13:15:26'),
(53, 'b06p54QFlUL3MfrcY5pUHTkyY9BkSd.jpg', 'Jay', 'Lalat', 'Marasigan', 'Davao City', 'Purok2', 'Rented', 'Female', 'AB+', 'Married', '2003-10-21', 'No', 'Elementary', 'Yes', 'IT', 'No', '091979123331', '', 'Roman Catholic', 'No', 'N/A', 'Yes', '1233', '13231', '', '2024-05-21 00:40:33', '2024-05-21 00:40:33'),
(54, '6q2hmcjnz6kFjGS4OfbLpZSz5lcf1u.png', 'Lee Christian', 'Ala', 'Amor', 'Davao City', 'Purok2', 'Owned', 'Male', 'B+', 'Single', '2002-12-18', 'No', 'College', 'Yes', 'IT', 'No', '091971233111', '', 'Atheism', 'No', 'N/A', 'Yes', '1233', '91', '', NULL, NULL),
(55, 'jxm0r33PA1zu335QBpaPOJUOYQmfKs.png', 'test', 'test', 'test', 'test', 'Purok1', 'Owned', 'Male', 'A+', 'Single', '2024-05-08', 'No', 'Elementary', 'Yes', 'test', 'Yes', '2312313', '', 'Roman Catholic', 'No', 'N/A', 'Yes', '123123', '13231', '', '2024-05-21 01:27:48', '2024-05-21 01:27:48');

--
-- Triggers `citizen`
--
DELIMITER $$
CREATE TRIGGER `UpdateCitizenStatus` AFTER INSERT ON `citizen` FOR EACH ROW BEGIN
...
CREATE TABLE `complaint` (
  ...
  `Complaints_status` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `fees` (
  ...
  `Citizen_ID` int(11) NOT NULL,
  `Staff_Name` varchar(100) NOT NULL,
  `Fee_Recipient` varchar(100) DEFAULT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `UpdateFeeTransactions` AFTER INSERT ON `fees` FOR EACH ROW BEGIN
...
CREATE TABLE `fee_type` (
  ...
  `Collection_period` varchar(50) DEFAULT NULL,
...
CREATE TABLE `fiscal_info` (
  ...
  `sk_fund` int(11) DEFAULT NULL,
  `rpt_share` int(11) DEFAULT NULL,
  `fees_and_charges` int(11) DEFAULT NULL,
  `others` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `fiscal_info`
--
...
CREATE TABLE `household` (
  ...
  `Worker_ID` int(11) NOT NULL,
  `family_member` varchar(150) NOT NULL,
  `Family_position` varchar(50) DEFAULT NULL,
...
--
DELIMITER $$
CREATE TRIGGER `UpdateHouseholdStatus` AFTER INSERT ON `household` FOR EACH ROW BEGIN
...
--
-- Stand-in structure for view `listallcomplaint`
-- (See below for the actual view)
--
CREATE TABLE `listallcomplaint` (
`Case_No` int(11)
,`Staff_ID` int(11)
,`Citizen_ID` int(11)
...
--
-- Stand-in structure for view `listdeceasedmembers`
-- (See below for the actual view)
--
CREATE TABLE `listdeceasedmembers` (
`Citizen_ID` int(11)
...
CREATE TABLE `political_info` (
  `Brgy_ID` int(11) NOT NULL,
  `LBC` text DEFAULT NULL,
  `No_of_precints` int(11) DEFAULT NULL,
  `Plebiscite_date` date DEFAULT NULL,
  `BDC_level` tinytext DEFAULT NULL,
  `BPOC_level` tinytext DEFAULT NULL,
  `BCPC_level` tinytext DEFAULT NULL,
  `VAW_desk_level` tinytext DEFAULT NULL,
  `BADAC_level` tinytext DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `political_info`
--
...
CREATE TABLE `purok` (
  ...
  `Purok_address` varchar(255) NOT NULL,
  `Purok_leader` varchar(100) NOT NULL,
  `Purok_population` int(11) DEFAULT NULL,
  `Worker_ID` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `residency_history` (
  ...
  `Residency_status` varchar(10) DEFAULT NULL,
...
--
-- Triggers `residency_history`
--
DELIMITER $$
CREATE TRIGGER `UpdateResidencyStatus` AFTER INSERT ON `residency_history` FOR EACH ROW BEGIN
...
--
-- Stand-in structure for view `retrieveactivebusinesses`
-- (See below for the actual view)
--
CREATE TABLE `retrieveactivebusinesses` (
`Business_ID` int(11)
...
--
-- Stand-in structure for view `retrieveallbarangaycaptainmembers`
-- (See below for the actual view)
--
CREATE TABLE `retrieveallbarangaycaptainmembers` (
`Admin_ID` int(11)
...
--
-- Stand-in structure for view `retrieveallcitizenmembers`
-- (See below for the actual view)
--
CREATE TABLE `retrieveallcitizenmembers` (
`Citizen_ID` int(11)
,`Citizen_fname` varchar(50)
,`Citizen_mname` varchar(50)
,`Citizen_lname` varchar(50)
...
--
-- Stand-in structure for view `retrievefeetransactions`
-- (See below for the actual view)
--
CREATE TABLE `retrievefeetransactions` (
`Fees_ID` int(11)
,`Staff_ID` int(11)
,`Citizen_ID` int(11)
,`Staff_Name` varchar(100)
...
--
-- Stand-in structure for view `retrievetransactionsbystaff`
-- (See below for the actual view)
--
CREATE TABLE `retrievetransactionsbystaff` (
...
('ZgOeDGrUdH9pfkO73305VGdLHERrGLyD8KGTuc73', 52, '::1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36', '', 1716532376);
...
CREATE TABLE `transaction` (
  ...
  `Transaction_citizen` varchar(100) DEFAULT NULL,
  `Trans_date` date DEFAULT NULL,
  `Trans_time` time(6) DEFAULT NULL,
  `Trans_status` text DEFAULT NULL,
  `Trans_purpose` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `transaction` (`Transaction_ID`, `Staff_Name`, `Transaction_citizen`, `Trans_date`, `Trans_time`, `Trans_status`, `Trans_purpose`) VALUES
(1, 'Emily Grace Johnson', 'John Doe Smith', '2024-04-25', '09:30:00.000000', 'Processed', 'Barangay Certification'),
(2, 'Matthew Thomas Garcia', 'Jane Mary Doe', '2024-04-26', '10:45:00.000000', 'Pending', NULL),
(3, 'Ethan Alexander Taylor', 'Michael James Johnson', '2024-04-27', '11:20:00.000000', 'Completed', NULL),
(4, 'Daniel Jacob Lee', 'Emily Grace Williams', '2024-04-27', '13:15:00.000000', 'Pending', NULL),
(5, 'Sophia Grace Hall', 'Matthew David Brown', '2024-04-28', '14:30:00.000000', 'Processed', NULL);

--
-- Triggers `transaction`
--
DELIMITER $$
CREATE TRIGGER `UpdateTransactionStatus` AFTER INSERT ON `transaction` FOR EACH ROW BEGIN 
...
CREATE TABLE `transaction_document` (
  ...
  `Template_tags` tinytext DEFAULT NULL,
  `Template_Settings` tinytext DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
-- Structure for view `basicinformationofmembers`
--
DROP TABLE IF EXISTS `basicinformationofmembers`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `basicinformationofmembers`  AS SELECT `citizen`.`Citizen_fname` AS `First_Name`, `citizen`.`Citizen_lname` AS `Last_Name`, `citizen`.`Address` AS `Address`, `citizen`.`Mobile_no` AS `Contact` FROM `citizen` ;

-- --------------------------------------------------------

--
-- Structure for view `listallcomplaint`
--
...
--
-- Structure for view `listdeceasedmembers`
--
...
--
-- Structure for view `retrieveactivebusinesses`
--
...
--
-- Structure for view `retrieveallbarangaycaptainmembers`
--
DROP TABLE IF EXISTS `retrieveallbarangaycaptainmembers`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `retrieveallbarangaycaptainmembers`  AS SELECT `barangay_admin`.`Admin_ID` AS `Admin_ID`, `barangay_admin`.`TermDuration` AS `TermDuration`, `barangay_admin`.`Designation` AS `Designation` FROM `barangay_admin` ;

-- --------------------------------------------------------

--
-- Structure for view `retrieveallcitizenmembers`
--
...
--
-- Structure for view `retrievefeetransactions`
--
...
--
-- Structure for view `retrievetransactionsbystaff`
--
...
--
-- Indexes for table `barangay_admin`
--
ALTER TABLE `barangay_admin`
  ADD PRIMARY KEY (`Admin_ID`);

--
-- Indexes for table `barangay_archives`
--
ALTER TABLE `barangay_archives`
...
--
-- Indexes for table `barangay_properties`
--
ALTER TABLE `barangay_properties`
...
--
ALTER TABLE `awards`
  MODIFY `award_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;

--
-- AUTO_INCREMENT for table `barangay_admin`
--
ALTER TABLE `barangay_admin`
  MODIFY `Admin_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;

--
-- AUTO_INCREMENT for table `barangay_archives`
--
ALTER TABLE `barangay_archives`
  MODIFY `Archive_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
...
--
-- AUTO_INCREMENT for table `barangay_properties`
--
ALTER TABLE `barangay_properties`
...
--
ALTER TABLE `barangay_workers`
  MODIFY `Worker_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=60;
...
--
ALTER TABLE `complaint`
  MODIFY `Case_No` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
...
--
ALTER TABLE `fees`
  MODIFY `Fees_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;

--
-- AUTO_INCREMENT for table `household`
--
ALTER TABLE `household`
  MODIFY `Household_No` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
...
--
ALTER TABLE `purok`
  MODIFY `Purok_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
